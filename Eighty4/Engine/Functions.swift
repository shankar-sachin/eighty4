...
enum Functions {
    ...
    private static func build() -> [String: FunctionDef] {
        ...
        t["ln"] = unary { if $0 <= 0 { throw CalcError.domain }; return log($0) }
        t["√"] = unary { if $0 < 0 { throw CalcError.domain }; return $0.squareRoot() }
        t["³√"] = unary { cbrt($0) }
        t["abs"] = unary { abs($0) }
        reg("abs", 1, 1) { a, _ in
        ...
        // LIST
        reg("sum", 1, 1) { a, _ in .num(try l(a[0]).reduce(0, +)) }
        reg("prod", 1, 1) { a, _ in .num(try l(a[0]).reduce(1, *)) }
        reg("mean", 1, 1) { a, _ in let x = try l(a[0]); guard !x.isEmpty else { throw CalcError.argument }; return .num(x.reduce(0, +) / Double(x.count)) }
        ...
        reg("dim", 1, 1) { a, _ in
            if case .matrix(let mm) = a[0] { let (r, c) = Matrix.dims(mm); return .list([Double(r), Double(c)]) }
            return .num(Double(try a[0].listValue().count))
        }
        reg("augment", 2, 2) { a, _ in
            if case .matrix(let x) = a[0], case .matrix(let y) = a[1] {
                guard x.count == y.count else { throw CalcError.dimMismatch }
                return .matrix(zip(x, y).map { $0 + $1 })
            }
        ...
        reg("ref", 1, 1) { a, _ in .matrix(Matrix.ref(try m(a[0]))) }
        reg("rref", 1, 1) { a, _ in .matrix(Matrix.rref(try m(a[0]))) }
